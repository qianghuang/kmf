...
file.write = function(filePath, contents, options){
	...
	console.log("file \"" + filePath + "\" is create.");
};
/**
 ...
 *
 */
file.copy = function(src, dest, hasBlankDir, callBack) {
	var encoding = "utf-8";
	var fileList;
	var _callback;
	...
	fileList.forEach(function(filename){
		...
		if(file.isDir(filename)) {
			...
			newContent = file.read(filename, {encoding: encoding});
			fileDirName = path.dirname(newPath);
			
			if(!file.exists(fileDirName)) {
				file.mkDir(fileDirName);
			}
			file.write(newPath,newContent, {encoding: encoding});
		}
	});
